...
export type DocumentTextResult = {
  ...
  readable: boolean;
  problems: string[];
  source: "provided-ocr" | "plain-text" | "pdf-text" | "docx-text" | "unreadable";
};
...
export function extractDocumentText(file: UploadedFile): DocumentTextResult {
  if (file.ocrText?.trim()) {
    return { text: cleanText(file.ocrText), readable: true, problems: [], source: "provided-ocr" };
  }
  ...
  if (!bytes.length) return unreadable("The document contained no readable bytes.");

  if (file.mimeType === "text/plain") {
    const text = cleanText(bytes.toString("utf8"));
    return text ? { text, readable: true, problems: [], source: "plain-text" } : unreadable("The text document was empty.");
  }
  ...
  const text = cleanText(bytes.toString("utf8"));
  return text ? { text, readable: true, problems: [], source: "plain-text" } : unreadable("Unsupported document format or no readable text.");
}
...
function extractPdfText(bytes: Buffer): DocumentTextResult {
  ...
  while ((match = streamPattern.exec(source))) {
    ...
    for (const candidate of [raw, tryInflate(raw, true), tryInflate(raw, false)]) {
  ...
  return unreadable("No selectable text was found. OCR is required for this scanned or image-only PDF.", "unreadable");
}
...
function extractPdfStrings(stream: string): string[] {
  ...
  let match: RegExpExecArray | null;
  while ((match = stringPattern.exec(stream))) out.push(decodePdfString(match[1]));
...
function extractDocxText(bytes: Buffer): DocumentTextResult {
  ...
  if (!documentXml) return unreadable("The DOCX document did not contain word/document.xml.", "docx-text");
  const text = cleanText(documentXml.toString("utf8").replace(/<w:tab\s*\/?>(?=\S)/g, " ").replace(/<w:br\s*\/?>(?=\S)/g, "\n").replace(/<[^>]+>/g, " "));
  return text ? { text, readable: true, problems: [], source: "docx-text" } : unreadable("The DOCX document contained no readable text.", "docx-text");
}
...
function unreadable(problem: string, source: DocumentTextResult["source"] = "unreadable"): DocumentTextResult {

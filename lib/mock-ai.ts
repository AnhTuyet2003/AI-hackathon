import { affirmativeText, applicationOnly, usable } from "./evidence-text";
...
const SPECIALIZATION_KEYWORDS: Array<{
  pattern: RegExp;
  specialization: string;
}> = [
  {
    pattern: /appendicitis|peritonitis|appendectomy|general surgery/i,
    specialization: "General Surgery",
  },
  { pattern: /asthma|pulmonology/i, specialization: "Pulmonology" },
  {
    pattern: /root canal|dental|pulpitis|tooth extraction/i,
    specialization: "Dental",
  },
  {
    pattern: /type\s?2 diabetes|diabetes|hba1c/i,
    specialization: "Endocrinology",
  },
  {
    pattern: /myocardial infarction|cardiac|heart|hypertension|coronary/i,
    specialization: "Cardiology",
  },
  {
    pattern: /cancer|tumor|oncology|carcinoma|chemotherapy/i,
  ...
  {
    pattern: /stroke|neurological|seizure/i,
    specialization: "Complex Medical",
  },
];

export function extractEntities(
  input: Pick<ApplicationInput, "medicalHistory" | "disclosures">,
  clinicalFields?: ExtractedFields,
): NERResult {
  const text = affirmativeText(
    `${applicationOnly(input.medicalHistory)}. ${applicationOnly(input.disclosures)}. ${[clinicalFields?.diagnosis, clinicalFields?.department, clinicalFields?.procedures, clinicalFields?.investigations].filter(Boolean).join(". ")}`,
  );
...
export function scoreComplexity(
  ...
  const applicationNer = extractEntities(input);
  const applicationDrivers: string[] = [];
  let applicationScore = 1;

  if (input.sumAssured > 500_000) {
    applicationScore += 3;
    applicationDrivers.push(
      `Sum Assured $${input.sumAssured.toLocaleString("en-US")} > $500K`,
    );
  } else if (input.sumAssured > 150_000) {
    applicationScore += 1;
    applicationDrivers.push(
      `Sum Assured $${input.sumAssured.toLocaleString("en-US")} > $150K`,
    );
  }
  ...
  const occupationHit = HIGH_RISK_OCCUPATIONS.find((o) =>
    input.occupation.toLowerCase().includes(o),
  );
  if (occupationHit) {
    applicationScore += 2;
    applicationDrivers.push(
      `Unusual/high-risk occupation: ${input.occupation}`,
    );
  }

  if (applicationNer.specialtiesRequired.length >= 2) {
    applicationScore += 3;
    applicationDrivers.push(
      `Multi-morbidity: ${applicationNer.specialtiesRequired.join(" + ")}`,
    );
  } else if (applicationNer.specialtiesRequired.length === 1) {
    applicationScore += 2;
    applicationDrivers.push(
      `Medical history: ${applicationNer.specialtiesRequired[0]}`,
    );
  }
    ...
    clinicalFields === undefined
      ...
      : Math.max(
          ...
          Math.min(
            10,
            Math.round(
              INTAKE_POLICY.applicationWeight * applicationScore +
                INTAKE_POLICY.clinicalWeight * clinical.score,
            ),
          ),
        );
...
export function scoreClinicalComplexity(
  ...
  if (fields === undefined)
    return {
      ...
      confidence: 0.9,
      evidence: [
        "No uploaded clinical document; application-only complexity path.",
      ],
    };
  ...
  const text = affirmativeText(
    Object.entries(fields)
      .filter(
        ([key, value]) =>
          typeof value === "string" &&
          !["relevantMedicalHistory", "medicalSummary"].includes(key),
      )
      .map(([, value]) => value)
      .join(". "),
  ).toLowerCase();
  ...
  let score = 0;
  const dentalCase =
    /oral health|dental|tooth|molar|gum|root canal|restoration|crown/i.test(
      text,
    );
  ...
  const severity = dentalCase
    ? [
        [
          /abscess|cellulitis|osteomyelitis|systemic infection|complication|emergency/i,
          2,
          "Dental complication or urgent presentation",
        ],
        [/multi-day|admission|inpatient/i, 1, "Extended or inpatient care"],
      ]
    : [
        [
          /severe|acute|peritonitis|sepsis|complication|unstable|emergency/i,
          2,
          "Acute or severe clinical presentation",
        ],
        [
          /admission|inpatient|operative|surgery|surgical|intravenous|multi-day/i,
          1,
          "Hospital-level or invasive care",
        ],
      ];
  let severityPoints = 0;
  for (const [pattern, points, label] of severity as [
    RegExp,
    number,
    string,
  ][]) {
    if (pattern.test(text)) {
      severityPoints = Math.min(3, severityPoints + points);
      evidence.push(label);
    }
  }
  ...
  if (dentalCase && /root canal|extraction|restoration|crown/i.test(text)) {
    score += 1;
    evidence.push(
      "Dental procedure documented without automatic high-complexity escalation",
    );
  } else if (
    /surgery|surgical|operative|appendectomy|invasive|procedure/i.test(text)
  ) {
    score += 2;
  ...
  if (
    /\bct\b|\bmri\b|ultrasound|radiograph|imaging|biopsy|rapid test|laboratory|lab result/i.test(
      text,
    )
  ) {
    score += /\bct\b|\bmri\b|biopsy|multiple|follow-up laboratory/i.test(text)
      ? 2
      : 1;
  ...
  if (
    fields.serviceStart &&
    fields.serviceEnd &&
    Date.parse(fields.serviceEnd) - Date.parse(fields.serviceStart) >= 86400000
  ) {
    score += 1;
    evidence.push("Duration or multi-day clinical course documented");
  }
  if (
    ner.specialtiesRequired.length >= 2 ||
    /general surgery|internal medicine/i.test(text)
  ) {
    score += 1;
    evidence.push(
      `Specialty involvement: ${ner.specialtiesRequired.join(", ") || "clinical specialty"}`,
    );
  }
  if (
    /unclear|ambiguous|contradict|unknown|incomplete|missing|complication|peritonitis/i.test(
      text,
    )
  ) {
    score += 1;
    evidence.push("Complication, ambiguity or contradiction requires review");
  }
  ...
  const heuristicConfidence =
    fieldCount === 0 ? 0.3 : fieldCount >= 8 ? 0.9 : 0.65;
  const requiredEvidenceCount = [
    ...
    typeof value === "string"
      ? value.trim().length > 0 &&
        !/^(?:not available|unavailable|unknown|not provided|none|n\/a|na)$/i.test(
          value.trim(),
        )
      : Boolean(value),
  ).length;
  const evidenceConfidence =
    requiredEvidenceCount < 4 ? 0.45 : heuristicConfidence;
  const confidence =
    fields.extractionConfidence == null
      ? evidenceConfidence
      : Math.min(
          evidenceConfidence,
          Math.max(0, Math.min(1, fields.extractionConfidence)),
        );
  return { score: Math.max(1, Math.min(10, score)), confidence, evidence };
}

export function detectMissingFields(input: ApplicationInput): {
  ...
  if (!input.medicalHistory.trim())
    missing.push("medical history / doctor notes");
  if (input.sumAssured > 200_000 && !input.disclosures.trim())
    missing.push("financial disclosures");
